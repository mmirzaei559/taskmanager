...
const TaskManager = () => {
    ...
    // Fetch tasks with error handling and cleanup
    ...
    const toggleTaskStatus = async (id, currentStatus) => {
        try {
            await axios.post(`${API_BASE_URL}/tasks/update`, {
                id,
    ...
    const runBenchmark = async () => {
        try {
            await axios.get(`${API_BASE_URL}/benchmark?count=1000`);
            alert('Benchmark completed successfully!');
            await fetchTasks();
        } catch (err) {
            setError('Benchmark failed: ' + (err.response?.data?.message || 'Server error'));
        }
    };
